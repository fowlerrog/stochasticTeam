...
for line in lines:
	try:
		...
		indVars = (nums[0]) # num targets
		if indVars in data:
			data[indVars].append([nums[5], nums[3], nums[6]]) # compute time, mean mission time, failure rate
		else:
			data[indVars] = [[nums[5], nums[3], nums[6]]]
	except:
