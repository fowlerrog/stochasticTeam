...
if __name__ == '__main__':
	...
	# for each results folder, plot plan_path_results time and any available execute_results times
	for s in sys.argv[1:]:
		try:
			...
			if not execTime is None:
				...
				if 'deterministic' in endFolderName.lower():
					detGroup[0].append(plannedTime)
					...
					if getVarFromString(endFolderName, 'REFINE_TOURS') == 'True':
						...
						stochGroupNoRefine[0].append(plannedTime)
						stochGroupNoRefine[1].append(execTime)
						stochGroupNoRefine[2].append(endFolderName)
				else:
